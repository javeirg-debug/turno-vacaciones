...
function agruparPermisos(permisos: Permiso[]) {
  ...
  permisos.forEach((permiso) => {
    ...
    // Buscar si ya existe un grupo del mismo usuario y tipo
    const gruposExistentes = resultado.filter(
      (item) =>
        "dias" in item &&
    ...
    // Buscar un grupo cuya fecha esté a 30 días o menos
    const fechaPermiso = new Date(
      permiso.fecha_inicio + "T00:00:00"
    );

    const grupo = gruposExistentes.find((grupo) => {
      return grupo.dias.some((dia) => {
    ...
    // Si no existe grupo compatible, creamos uno
    if (!grupo) {
      resultado.push({
        id: permiso.id,
...
// Solo permisos anteriores a hoy
const hoy = new Date();
hoy.setHours(0, 0, 0, 0);

const listaPasada = lista.filter((permiso) => {
...
setPermisos(listaPasada);

setPermisosVista(
  agruparPermisos(listaPasada)
);
      } catch (err) {
        console.error(
